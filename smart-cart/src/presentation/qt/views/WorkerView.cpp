...
WorkerView::WorkerView(WorkerViewModel& viewModel, QWidget* parent)
    ...
    , dialogManager_(new WorkflowDialogManager(this, viewModel))
{
    setupUi();
    connectSignals();
...
void WorkerView::showEvent(QShowEvent* event) {
    ...
    QTimer::singleShot(900, this, [this]() {
        ...
        if (topLevel && topLevel->isVisible() && isVisible()) {
            dialogManager_->setModalPopupsReady(true);
        }
    });
}
...
void WorkerView::onActiveModuleAvailabilityChanged(bool online) {
    ...
    dialogManager_->setModuleOnline(online);
    updateVisiblePage();
}
...
void WorkerView::onImportOrderClicked() {
    ...
    const bool skipFeederLoading = dialogManager_->askCartWasAlreadyInWork();
