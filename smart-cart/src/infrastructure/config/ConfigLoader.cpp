...
AppConfig ConfigLoader::loadFromFile(const std::string& path) {
    ...
    cfg.logFilePath = cfg.logFile;
    cfg.sqliteDbPath = cfg.sqlitePath;
...
void ConfigLoader::validate(AppConfig& cfg) {
    if (cfg.logFile.empty()) throw std::runtime_error("ConfigLoader: log_file is empty");
    if (cfg.sqlitePath.empty()) throw std::runtime_error("ConfigLoader: sqlite_path is empty");
    if (cfg.stm32PollMs == 0) throw std::runtime_error("ConfigLoader: stm32_poll_ms must be > 0");
    if (cfg.debounceMs == 0) throw std::runtime_error("ConfigLoader: debounce_ms must be > 0");
    if (cfg.stableConfirmMs < cfg.debounceMs) {
        throw std::runtime_error("ConfigLoader: stable_confirm_ms must be >= debounce_ms");
    }
    if (!cfg.slotToLedMap.empty() && cfg.slotToLedMap.size() != 24) {
        throw std::runtime_error("ConfigLoader: slot_to_led_map must contain 24 entries");
    }
}
