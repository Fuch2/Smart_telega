...
// Исправлено: frame.frameType → frame.type, frame.commandId → frame.cmdId
...
constexpr size_t kMinFrameSize    = 2 + kHeaderNoSofSize + 2;
...
uint16_t FrameCodec::crc16CcittFalse(const uint8_t* data,
...
std::vector<uint8_t> FrameCodec::encode(const Frame& frame) {
    ...
    out.push_back(static_cast<uint8_t>(frame.type));   // ← исправлено
    out.push_back(frame.seq);
    out.push_back(static_cast<uint8_t>(frame.cmdId));  // ← исправлено
...
std::optional<Frame> FrameCodec::decode(const uint8_t* data, size_t size) {
    if (!data || size < kMinFrameSize)           return std::nullopt;
    ...
    f.type   = static_cast<FrameType>(data[3]);   // ← исправлено
    f.seq    = data[4];
    f.cmdId  = static_cast<CommandId>(data[5]);   // ← исправлено
...
FrameCodec::StreamParser::StreamParser() { reset(); }
...
FrameCodec::ParseEvent FrameCodec::StreamParser::feed(uint8_t byte) {
    ...
    case State::CrcHi: {
        const uint16_t rx_crc =
            static_cast<uint16_t>(crc_lo_) |
            (static_cast<uint16_t>(byte) << 8U);
        const uint16_t calc_crc =
            crc16CcittFalse(crc_data_.data(), crc_data_.size());
        ...
        f.type   = static_cast<FrameType>(header_[1]);  // ← исправлено
        f.seq    = header_[2];
        f.cmdId  = static_cast<CommandId>(header_[3]);  // ← исправлено
        f.payload = payload_;
