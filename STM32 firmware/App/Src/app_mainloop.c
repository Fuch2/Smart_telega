...
void app_mainloop_step(void)
{
    ...
    if (period_elapsed(now, g_ml.last_switch_scan_ms, g_ml.cfg.switch_scan_period_ms)) {
        g_ml.last_switch_scan_ms = now;
        (void)switch_scan_get_raw_mask();
    }

    if (period_elapsed(now, g_ml.last_debounce_ms, g_ml.cfg.debounce_period_ms)) {
        g_ml.last_debounce_ms = now;
        /* текущий MVP smoke: debounce pipeline будет расширен в полной интеграции */
    }
